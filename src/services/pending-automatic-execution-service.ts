...
export interface ExecutionDispatcher { enqueueInitial(proposalId: string): Promise<void>; isReady(): boolean; }
...
export class PendingAutomaticExecutionService implements PendingAnalysisHook {
  ...
  async onPending(value: unknown, monitored: MonitoredAddress, chain?: EvmChainConfig): Promise<void> {
    ...
    if (!this.executionReady() || (this.dispatcher && !this.dispatcher.isReady())) { this.logger.warn({ transactionHash: transaction.transactionHash, proposalId: proposal.id }, 'Automatic execution is queued in PostgreSQL pending durable recovery/queue readiness'); return; }
    try { if (this.dispatcher) await this.dispatcher.enqueueInitial(proposal.id); else await this.executor.execute(proposal); }
